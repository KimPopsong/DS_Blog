...
import cicd from '../../assets/projects/GOLDEN_TICKET/GOLDENTICKET_CICD.jpg';
...
- Infra
    <br />
    <Image src={cicd} alt="GitLab CI/CD 파이프라인" />
...
## [사용자 인터뷰 기반 UX 개선]
- 빠르게 1주 만에 MVP를 제작하여 배포하고, 실제 타겟 사용자인 시니어(부모님 세대)를 대상으로 심층 인터뷰 진행
    - 심층 인터뷰 5명이면 사용성 문제의 80% 이상을 발견할 수 있다는 연구 결과를 참고하여, 이보다 많은 10명을 대상으로 진행
    - 개선 효과를 단계별로 검증하기 위해 총 3차에 걸쳐 인터뷰 진행
        1. 1차 : 별도 설명 없이 MVP를 사용하도록 요청
        2. 2차 : 보조자의 도움을 받아 MVP를 사용하도록 요청
        3. 3차 : 개선된 버전을 도움 없이 사용하도록 요청
- 1차 인터뷰에서 "티켓의 위치를 모르겠다", "버튼 크기가 작다" 등 UI 인지 및 접근성 관련 피드백 다수 수집
    - 시니어 사용자 특성상 작은 버튼과 복잡한 화면 구조에서 어려움을 느낀다는 점 확인
- 피드백을 반영해 주요 버튼 및 UI 요소의 크기를 확대하고, 사용자 동선을 고려해 시스템 구조를 단순화하여 티켓 관련 기능의 접근 경로를 직관적으로 재배치
- 개선 후 3차 인터뷰에서는 도움 없이도 서비스를 원활히 이용할 수 있다는 호평을 받으며 개선 방향의 유효성을 확인
- 이 과정을 통해 사용자의 요구를 분석해 서비스로 구현하는 방법과, 실사용 환경을 고려한 성능 최적화, 그리고 사용자 피드백 기반 개선 사이클의 중요성을 체감
