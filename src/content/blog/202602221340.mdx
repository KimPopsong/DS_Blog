---
title: "URLModifier 프로젝트 트러블슈팅 - Docker PostgreSQL 연결 오류"
...
- POSTGRES_USER / POSTGRES_PASSWORD 값이 application.yaml과 일치함을 확인
-> Docker 설정에는 문제가 없다고 판단
...
- Docker 컨테이너를 중지하고 이미지를 제거한 뒤에도 5432 포트가 LISTENING 상태임을 확인
...
## 해결 방법
- WSL 내부에서 PostgreSQL 서비스를 중지해서 5432 포트를 비워둠
- 이후 Docker PostgreSQL 컨테이너만 5432 포트를 사용하도록 유지
- 로컬 개발 환경에서는 먼저 `netstat` / `tasklist` 로 실제로 어떤 프로세스가 포트를 점유하고 있는지 확인한 후, Docker 컨테이너가 맞는지 확인하는 습관 들이기